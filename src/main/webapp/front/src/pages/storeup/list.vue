...
<div :style='{"width":"1200px","padding":"20px","boxShadow":"0px 4px 10px 0px rgba(0,0,0,0.302)","margin":"10px auto","position":"relative","background":"#fff"}'>
    ...
    <div class="section-title" :style='{"border":"4px solid rgba(197, 19, 19, 1)","margin":"10px 0","fontSize":"20px","lineHeight":"54px","color":"#333","textAlign":"center"}'>My Collection</div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getStoreupList(page) {
        let params = {page, limit: this.pageSize, type: 1, userid: localStorage.getItem('userid')};
        let searchWhere = {};
      ...
      toDetail(item) {
        this.$router.push({path: `/index/${item.tablename}Detail`, query: {isStoreup: 'Yes', detailObj: JSON.stringify(item)}});
      }
    }
  }
</script>
